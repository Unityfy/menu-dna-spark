...
const StepIngredientCosts = ({ data, onChange }: Props) => {
  ...
  const updateCostEntry = (entry: IngredientCostEntry) => {
    const total = entry.ingredients.reduce((s, i) => s + i.unitCost * (i.portionQty / 1000), 0);
  ...
  const activeEntry = getCostEntry(activeDish);
  const completedCount = data.ingredientCosts.filter((c) => c.ingredients.some((i) => i.name)).length;

  return (
    <div className="space-y-5">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Accurate food costs reveal true dish profitability. Enter ingredients manually or upload a cost CSV.
      </p>
      ...
      {mode === "csv" ? (
        <div className="rounded-md border border-dashed border-border bg-secondary/30 p-6 text-center space-y-3">
          ...
          <p className="text-xs text-muted-foreground">Format: Dish Name, Ingredient, Unit Cost, Portion Qty, Unit</p>
        ...
        <>
          ...
          <div className="flex gap-2 flex-wrap">
            {items.map((item) => {
              const hasData = data.ingredientCosts.some((c) => c.dishId === item.id && c.ingredients.some((i) => i.name));
              return (
                <button
                  key={item.id}
          ...
          <div className="space-y-2">
            {activeEntry.ingredients.map((ing, idx) => (
              <div key={idx} className="grid grid-cols-12 gap-2 items-end">
                ...
                <div className="col-span-3 space-y-1">
                  {idx === 0 && <label className="text-xs text-muted-foreground">Portion (g)</label>}
                  <input
                    type="number"
            ...
            <button
              onClick={() => {
                const updated = { ...activeEntry };
                updated.ingredients = [...updated.ingredients, { name: "", unitCost: 0, portionQty: 0, unit: "g" }];
                updateCostEntry(updated);
              }}
          ...
          {activeEntry.totalFoodCost > 0 && (
            <div className="rounded-md border border-border bg-secondary/50 p-3 flex justify-between items-center">
